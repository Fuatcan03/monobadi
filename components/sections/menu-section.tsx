                ...
                <div className="flex items-center justify-between w-full px-4">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="font-semibold text-base">
                      {(dict.categories as any)[activeTab]}
                    </span>
                  </div>
              ...
              <TabsList className="flex flex-wrap justify-center gap-2 bg-transparent p-2 rounded-2xl">
                {categories.map((catKey) => (
                  <TabsTrigger 
                    key={catKey} 
                    value={catKey} 
                    className="group relative px-5 py-3 transition-all duration-300 rounded-lg min-w-[120px]"
                  >
                    {/* Background Effect */}
                    ...
                    {/* Border Effect */}
                    <div className="absolute inset-0 border-2 border-transparent group-data-[state=active]:border-primary/30 rounded-lg transition-all duration-300" />
                    
                    {/* Content */}
                    <div className="relative z-10 flex flex-col items-center gap-1.5">
                      <div className={`p-2 rounded-lg transition-colors duration-300 ${activeTab === catKey ? 'bg-primary/10 text-primary' : 'bg-muted text-muted-foreground'}`}>
                        {categoryIcons[catKey]}
                      </div>
              ...
              <Card className="p-4 md:p-8 rounded-2xl md:rounded-3xl shadow-lg md:shadow-xl border-primary/10 bg-card/60 backdrop-blur-sm">
                ...
                >
                  {(dict.menu as any)[activeTab]?.map((item: any, idx: number) => (
                    ...
                    >
                      ...
                      <div className="flex flex-col sm:flex-row justify-between items-start gap-4 p-4 rounded-xl border border-muted-foreground/10 hover:border-primary/20 hover:shadow-md transition-all duration-300 bg-gradient-to-br from-white/50 to-white/30 dark:from-card/50 dark:to-card/30">
                        {/* Ürün Görseli */}
                        <div className="flex-shrink-0">
                          <div className="w-20 h-20 md:w-24 md:h-24 rounded-xl overflow-hidden border border-primary/10 bg-gradient-to-br from-muted to-muted/50 shadow-sm group-hover:shadow-md transition-all duration-500">
                            <img 
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex justify-between items-start gap-2 mb-2">
                            <h4 className="font-bold text-lg md:text-xl group-hover:text-primary transition-colors leading-tight line-clamp-2 pr-2">
                              {item.name}
                            </h4>
                            <div className="flex flex-col items-end">
                              <span className="font-bold text-primary text-xl md:text-2xl whitespace-nowrap">
                                {item.price}
                              </span>
                ...
                <div className="mt-6 pt-4 border-t border-primary/10">
                  <p className="text-sm text-muted-foreground text-center">
                    {language === "tr" ? (
                      <>{categories.length} kategori, toplam <span className="font-semibold text-primary">{(dict.menu as any)[activeTab]?.length}</span> ürün</>
                    ) : (
                      <>{categories.length} categories, total <span className="font-semibold text-primary">{(dict.menu as any)[activeTab]?.length}</span> items</>
                    )}
                  </p>
                </div>
              </Card>
